...
func (r *BarbicanKeystoneListenerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	Log := r.GetLogger(ctx)
	secretFn := func(_ context.Context, o client.Object) []reconcile.Request {
		...
		// get all barbican keystone listener CRs
		listeners := &barbicanv1beta1.BarbicanKeystoneListenerList{}
		listOpts := []client.ListOption{
			client.InNamespace(namespace),
		}
		if err := r.Client.List(context.Background(), listeners, listOpts...); err != nil {
			Log.Error(err, "Unable to retrieve barbicankeystonelistener CRs %v")
			return nil
		}
		// Watch for changes to any CustomServiceConfigSecrets
		for _, cr := range listeners.Items {
			for _, v := range cr.Spec.CustomServiceConfigSecrets {
				if v == secretName {
					...
					Log.Info(fmt.Sprintf("Secret %s is used by BarbicanKeystoneListener CR %s", secretName, cr.Name))
