...
func (r *BarbicanAPIReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	Log := r.GetLogger(ctx)
	secretFn := func(_ context.Context, o client.Object) []reconcile.Request {
		...
		// get all barbican API CRs
		apis := &barbicanv1beta1.BarbicanAPIList{}
		listOpts := []client.ListOption{
			client.InNamespace(namespace),
		}
		if err := r.Client.List(context.Background(), apis, listOpts...); err != nil {
			Log.Error(err, "Unable to retrieve barbicanapi CRs %v")
			return nil
		}
		...
		for _, cr := range apis.Items {
			for _, v := range cr.Spec.CustomServiceConfigSecrets {
				if v == secretName {
					...
					Log.Info(fmt.Sprintf("Secret %s is used by BarbicanAPI CR %s", secretName, cr.Name))
