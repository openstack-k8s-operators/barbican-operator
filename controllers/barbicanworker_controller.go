...
func (r *BarbicanWorkerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	Log := r.GetLogger(ctx)
	secretFn := func(_ context.Context, o client.Object) []reconcile.Request {
		...
		// get all barbican worker CRs
		workers := &barbicanv1beta1.BarbicanWorkerList{}
		listOpts := []client.ListOption{
			client.InNamespace(namespace),
		}
		if err := r.Client.List(context.Background(), workers, listOpts...); err != nil {
			Log.Error(err, "Unable to retrieve barbicanworker CRs %v")
			return nil
		}
		// Watch for changes to any CustomServiceConfigSecrets
		for _, cr := range workers.Items {
			for _, v := range cr.Spec.CustomServiceConfigSecrets {
				if v == secretName {
					...
					Log.Info(fmt.Sprintf("Secret %s is used by BarbicanWorker CR %s", secretName, cr.Name))
