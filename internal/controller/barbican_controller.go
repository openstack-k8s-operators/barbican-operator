...
func (r *BarbicanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	if instance.Spec.NotificationsBus != nil {
		c := condition.UnknownCondition(
			barbicanv1beta1.BarbicanNotificationBusReadyCondition,
			condition.InitReason,
			barbicanv1beta1.BarbicanNotificationBusReadyInitMessage)
		cl.Set(c)
	}
	...
	if instance.Spec.NotificationsBus != nil {
		...
		if err != nil {
			instance.Status.Conditions.Set(condition.FalseCondition(
				...
				condition.ErrorReason,
				condition.SeverityWarning,
				barbicanv1beta1.BarbicanNotificationBusReadyErrorMessage,
				err.Error()))
			return ctrl.Result{}, err
		}
		...
		*instance.Status.NotificationsURLSecret = notificationBusInstanceURL.Status.SecretName
		...
		if *instance.Status.NotificationsURLSecret == "" {
			...
			instance.Status.Conditions.Set(condition.FalseCondition(
				...
				condition.RequestedReason,
				condition.SeverityInfo,
				barbicanv1beta1.BarbicanNotificationBusReadyRunningMessage))
			return ctrl.Result{RequeueAfter: time.Duration(10) * time.Second}, nil
		}

		instance.Status.Conditions.MarkTrue(barbicanv1beta1.BarbicanNotificationBusReadyCondition, barbicanv1beta1.BarbicanNotificationBusReadyMessage)
	} else {
...
func (r *BarbicanReconciler) transportURLCreateOrUpdate(
	...
	if rabbitmqConfig != nil {
		rmqName = fmt.Sprintf("%s-barbican-transport-%s", instance.Name, rabbitmqConfig.Cluster)
